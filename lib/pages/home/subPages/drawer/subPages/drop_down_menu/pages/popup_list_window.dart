...
class PopupWindow extends StatefulWidget {
  ...
  /// 依附的组件和 PopUpWindow 组件共同持有的 GlobalKey
  ...
  /// popUpWindow 位于 targetView 的方向，默认为 [PopupDirection.bottom]
...
class DCPopupRoute extends PopupRoute {
  final Duration _duration = const Duration(milliseconds: 200);
  Widget child;

  DCPopupRoute({required this.child});
...
class PopupListWindow {
  ...
  /// [popKey] 依附的组件和PopUpWindow组件共同持有的GlobalKey
  /// [data] 要显示的文本数据列表
  /// [popDirection] 箭头的方向
  ...
  static double _getMaxWidth(TextStyle textStyle, List<String> data) {
    ...
    for (String entity in data) {
      Size size = textSize(entity, textStyle);
  ...
  static Size textSize(String text, TextStyle style) {
    ...
    if (text == null) return const Size(0, 0);
